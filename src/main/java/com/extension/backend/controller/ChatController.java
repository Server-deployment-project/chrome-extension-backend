...
public class ChatController {
    ...
    public ResponseEntity<Flux<String>> chat(@RequestBody ChatRequest request, ServerWebExchange exchange) {
        ...
        if ((request.getContent() == null || request.getContent().isEmpty()) &&
            ...
            throw BusinessException.badRequest("content or messages is required");
        }

        // 验证 action
        if (request.getAction() != null && !request.getAction().equals("analyze")
                && !request.getAction().equals("chat")) {
            throw BusinessException.badRequest("action must be 'analyze' or 'chat'");
        }
        ...
        try {
            ...
            // 调用 LLM 服务
            Flux<String> stream = siliconFlowService.chatStream(
                    messages,
                    ...
                    .onErrorResume(error -> {
                        ...
                        return Flux.just(
                                "data: {\"error\":{\"message\":\"" + errorMsg + "\",\"code\":\"upstream_error\"}}\n\n");
                    });
